...
async function makePayment(req, res) {
  try {
    ...
    if (idempotencyKey === undefined) {
      ...
      ErrorResponse.error = new AppError(['Idempotency key is missing'], StatusCodes.BAD_REQUEST);
    ...
    if (inMemoryDB[idempotencyKey]) {
      ErrorResponse.message = 'Something went wrong';
      ErrorResponse.error = new AppError(['The payment is already done for this booking'], StatusCodes.BAD_REQUEST);
